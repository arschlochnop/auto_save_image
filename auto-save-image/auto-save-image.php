...
Version: 2.3
Plugin URI: http://www.ha97.com/1228.html 
Description: 自动保存远程图片
Authorship: Bai Yunshan
...
function Auto_Save_Image_savepost($content){
	...
	if($_POST['Auto_Save_Image']=="1"){
		...
		foreach ($img_array as $key => $value){
			...
			if(str_replace(get_bloginfo('url')."/wp-content/uploads/".$photo_savepath,"",$value)==$value){
			/*
				对于本地图片打完水印产生冗余,未完成
				...
				// preg_match("/.*\/([\s\S]*\.\S{3}$)/",$value,$filename_array);
			*/
